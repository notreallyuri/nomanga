...
# Re-run after moving the repo; a plain rebuild needs no re-run (the symlink
# points at target/release, so the new binary is picked up in place).
set -euo pipefail
...
# launching through a symlink named nomanga is what makes the launcher match
# the window to this .desktop file.
install -d "$HOME/.local/bin"
ln -sfn "$bin" "$HOME/.local/bin/nomanga"
...
echo "installed: ~/.local/bin/nomanga -> $bin"
